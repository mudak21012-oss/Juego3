...
export default class Tile {
  ...
  public doorOrientation?: 'horizontal' | 'vertical'; // Para marcar orientación de puertas
  ...
  private rawIndex(): number {
    ...
    if (this.type === TileType.Door) {
      // Usar la orientación marcada explícitamente en lugar de vecinos
      if (this.doorOrientation === 'horizontal') {
        return Graphics.environment.indices.doors.horizontal; // 0x81 para norte/sur
      } else {
        return Graphics.environment.indices.doors.vertical; // 0x90 para este/oeste
      }
    }
